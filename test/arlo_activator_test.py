...
class arlo_activator_test(unittest.TestCase):
    ...
    def test_disarmed_armed_should_be_dirty(self):
        self.activator.alarm.status = MagicMock(return_value = {"StatusAnnex": "disarmed", "AnnexAvailable": None, "AlarmStatus": "armed"})
        ...
        self.assertEqual(alarm_status.state['AlarmStatus'], stored['AlarmStatus'], 'Stored state is wrong, should be armed.')
    
    def test_disarmed_armed_should_be_not_dirty(self):
        self.activator.alarm.status = MagicMock(return_value = {"StatusAnnex": "disarmed", "AnnexAvailable": None, "AlarmStatus": "disarmed"})
        self.__write_disarmed()
        alarm_status = self.activator.get_alarm_status()
